...
class process_cell():
    def __init__(self):
        ...
        self.cell_encoder = './process_data/TCGA_GDSC_enc.csv'
        self.cell_expression = './process_data/TCGA_GDSC_exp.csv'
...
pred_dti_pdtc=pd.read_csv('../GCADTI/pred_dti_tcga.csv',index_col=0)
pred_dti_test=[]
...
gene=pd.read_csv('./process_data/TCGA_GDSC_enc.csv',index_col=0)
col=gene.columns
...
class Model:
    def __init__(self,modeldir,model,device,epoch):
        self.model = modeldict[model](pred_dti.shape[1],596,'./gene2vec_595.npy',2934)
...
test_pairs=pd.read_csv('./process_data/tcga_response.csv',index_col=0)
test_pairs=test_pairs[test_pairs.bcr_patient_barcode.isin(pd.read_csv('./process_data/TCGA_GDSC_enc.csv',index_col=0).index)]
